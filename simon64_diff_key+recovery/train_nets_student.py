...
from tensorflow.keras.layers import Dense, Conv1D, Input, Reshape, Add, Flatten, BatchNormalization, Activation, \
    Permute, GlobalAveragePooling1D, multiply
...
# bs = 1000
wdir = './freshly_trained_student_nets/11-round-ND/'
diff = (0x0, 0x80000000)
bits = [31, 30, 29, 28, 27, 26, 25, 24, 23, 22, 21, 20, 19, 18, 17]
teacher = './freshly_trained_nets/s_groups=4/11-round-ND/best11depth5(0x0,80000000).h5'
...
def make_resnet(num_blocks=2, num_filters_1=32, num_filters_2=32, num_filters_3=32, num_outputs=1, d0=512, d1=64, d2=64,
                word_size=32, ks=3, depth=5, reg_param=0.0001, final_activation='sigmoid', s_groups=1, ks_value_1=1,
                ks_value_2=3, ks_value_3=3, dropout_value=0.8, se_ratio=16):
    ...
    for i in range(depth):
        conv1 = Conv1D(num_filters_2, kernel_size=ks_value_2, padding='same', kernel_regularizer=l2(reg_param))(
            shortcut)
...
def extract_sensitive_bits(raw_x, bits=[20, 19, 18, 17, 16, 15, 14, 13, 12, 11, 10, 9, 8]):   #len(bits)*8*s_gropps
    # get new-x according to sensitive bits
    id0 = [sp.WORD_SIZE() - 1 - v for v in bits]
    #print('id0 is ', id0)
    id1 = [v + sp.WORD_SIZE() * i for i in range(32) for v in id0]     #8*s_gropps
    #print('id1 is ', id1)
    new_x = raw_x[:, id1]
...
def train_distinguisher(num_epochs, num_rounds=7, depth=1, filename="default", s_groups=1, bs=5000, lr_choice="14",
                        d0=512, d1=64, d2=64, ks_value_1=1, ks_value_2=3, ks_value_3=3, machine_name='vgc',
                        dropout_value=0.8, num_filters_1=32, num_filters_2=32, num_filters_3=32, reg_param=10**-4,
                        high_lr=0.002, low_lr=0.0001, lr_epoch=10, se_ratio=16):
    ...
    X, Y = sp.make_train_data(10**7, num_rounds, diff=diff, s_groups=s_groups)   #减少一些数据量
    predicted_y = teacher_net.predict(X, batch_size=bs)
    extracted_x = extract_sensitive_bits(X, bits=bits)
    predicted_y[predicted_y > 0.5] = 1
    predicted_y[predicted_y <= 0.5] = 0

    # generate eval data
    X_eval, Y_eval = sp.make_train_data(10**6, num_rounds, diff=diff, s_groups=s_groups)
    extracted_x_eval = extract_sensitive_bits(X_eval, bits=bits)

    net = make_resnet(word_size=len(bits), depth=depth, reg_param=reg_param, s_groups=s_groups, d0=d0, d1=d1, d2=d2, ks_value_1=ks_value_1,
                      ks_value_2=ks_value_2, ks_value_3=ks_value_3, dropout_value=dropout_value,
                      num_filters_1=num_filters_1, num_filters_2=num_filters_2, num_filters_3=num_filters_3,
                      se_ratio=se_ratio)
    # net.compile(optimizer='Adam',loss='mse',metrics=['acc'])
    net.compile(optimizer='Adam', loss='mse',
                metrics=['acc', tf.keras.metrics.TruePositives(), tf.keras.metrics.TrueNegatives(),
    ...
    h =net.fit(extracted_x, predicted_y, epochs=num_epochs, batch_size=bs, verbose=1, validation_data=(extracted_x_eval, Y_eval), callbacks=[lr, check])

    import datetime
    curr_time = datetime.datetime.now()
    filename = "ep{0}_round{1}_depth{2}_group{3}_bs{4}_lr{5}_d1{6}_d2{7}_ks{8}_dp{9}_filter{10}_reg{11}_lr{12}_{13}_lrepoch{14}_{15}".format(
        str(num_epochs), str(num_rounds), str(depth), str(s_groups), str(bs), str(lr_choice), str(d1), str(d2),
        str(ks_value_1) + str(ks_value_2) + str(ks_value_3), str(dropout_value)[2:],
        str(num_filters_1) + "-" + str(num_filters_2) + "-" + str(num_filters_3), str(reg_param),
        str(high_lr) + "-" + str(low_lr), machine_name, str(lr_epoch),
        str(curr_time.year) + '-' + str(curr_time.month) + '-' + str(curr_time.day) + '-' + str(
            curr_time.hour) + '-' + str(curr_time.minute) + '-' + str(curr_time.second))
    import os
    import pandas as pd

    repeat_filename = "ep{0}_round{1}_depth{2}_group{3}_bs{4}_lr{5}_d1{6}_d2{7}_ks{8}_dp{9}_filter{10}_reg{11}_lr{12}_{13}_lrepoch{14}".format(
        str(num_epochs), str(num_rounds), str(depth), str(s_groups), str(bs), str(lr_choice), str(d1), str(d2),
        str(ks_value_1) + str(ks_value_2) + str(ks_value_3), str(dropout_value)[2:],
        str(num_filters_1) + "-" + str(num_filters_2) + "-" + str(num_filters_3), str(reg_param),
        str(high_lr) + "-" + str(low_lr), machine_name, str(lr_epoch))
    ...
    new_df = {
        ...
        'd0': [d0],
        'curr_time': [str(curr_time.year) + '-' + str(curr_time.month) + '-' + str(curr_time.day) + '-' + str(
            curr_time.hour) + '-' + str(curr_time.minute) + '-' + str(curr_time.second)],
