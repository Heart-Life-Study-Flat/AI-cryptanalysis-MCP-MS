...
# bs = 1000
wdir = './freshly_trained_nets/s_groups=4/11-round-ND/'
...
def train_distinguisher(num_epochs, num_rounds=7, depth=1, filename="default", s_groups=1, bs=5000, lr_choice="14",
    ...
    net = make_resnet(depth=depth, reg_param=reg_param, s_groups=s_groups, d0=d0, d1=d1, d2=d2, ks_value_1=ks_value_1,
    ...
    '''
    X, Y = sp.make_train_data(10 ** 7, num_rounds, s_groups=s_groups)
    X_eval, Y_eval = sp.make_train_data(10 ** 6, num_rounds, s_groups=s_groups)
    ...
    h = net.fit(X, Y, epochs=num_epochs, batch_size=bs, verbose=1, validation_data=(X_eval, Y_eval),
                callbacks=[lr, check])
