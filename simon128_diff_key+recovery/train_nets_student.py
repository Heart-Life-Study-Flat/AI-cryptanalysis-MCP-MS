...
#bs = 1000
#wdir = './freshly_trained_nets/key-recovery/17-round-ND/'
#wdir = './freshly_trained_nets/key-recovery/18-round-ND/'
wdir = './freshly_trained_student_nets/17-round-ND/'
diff = (0x0, 0x8000000000000)
bits = [51, 50, 49, 48, 47, 46, 45, 44, 43, 42, 41, 40, 39]
teacher = './freshly_trained_nets/key-recovery/17-round-ND/best17depth5-(0x0,8000000000000).h5'
...
def make_resnet(num_blocks=2, num_filters_1=64, num_filters_2=64, num_filters_3=64, num_outputs=1, d0=512,d1=128, d2=128, word_size=64, ks=3,depth=5, reg_param=0.0001, final_activation='sigmoid',s_groups = 1,ks_value_1=1,ks_value_2=3,ks_value_3=3,dropout_value=0.8,se_ratio=16):
...
def extract_sensitive_bits(raw_x, bits=[20, 19, 18, 17, 16, 15, 14, 13, 12, 11, 10, 9, 8]):   #len(bits)*8*s_gropps
    ...
    #print('id0 is ', id0)
    id1 = [v + sp.WORD_SIZE() * i for i in range(24) for v in id0]     #8*s_gropps
    #print('id1 is ', id1)
    new_x = raw_x[:, id1]
...
def train_distinguisher(num_epochs, num_rounds=7, depth=1,filename="default",s_groups=1,bs=5000,lr_choice="14",d0=512,d1=128,d2=128,ks_value_1=1,ks_value_2=3,ks_value_3=3,machine_name='vgc',dropout_value=0.8,num_filters_1=64,num_filters_2=64,num_filters_3=64,reg_param=10**-4,high_lr=0.002,low_lr=0.0001,lr_epoch=10,se_ratio=16):
    ...
    X, Y = sp.make_train_data(10**6*2, num_rounds, diff=diff, s_groups=s_groups)   #减少一些数据量,由10**7*2将为10**7
    ...
    # generate eval data
    X_eval, Y_eval = sp.make_train_data(10**5*2, num_rounds, diff=diff, s_groups=s_groups)  #减少一些数据量,由10**6*2将为10**6
    extracted_x_eval = extract_sensitive_bits(X_eval, bits=bits)

    net = make_resnet(word_size=len(bits), depth=depth, reg_param=reg_param,s_groups=s_groups,d0=d0,d1=d1,d2=d2,ks_value_1=ks_value_1,ks_value_2=ks_value_2,ks_value_3=ks_value_3,dropout_value=dropout_value,num_filters_1=num_filters_1,num_filters_2=num_filters_2,num_filters_3=num_filters_3,se_ratio=se_ratio)
    ...
    # 减少数据量
    #X, Y = sp.make_train_data(10**7, num_rounds, s_groups=s_groups)    #数据量降了2倍，10**7*2降为10**7
    #X_eval, Y_eval = sp.make_train_data(10**6, num_rounds, s_groups=s_groups)   #数据量降了2倍，10**6*2降为10**6
