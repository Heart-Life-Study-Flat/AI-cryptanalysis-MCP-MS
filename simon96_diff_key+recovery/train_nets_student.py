...
#bs = 1000
#wdir = './freshly_trained_nets/key-recovery/15-round-ND/'   #最后一个斜杠可以起到一个新建文件夹的作用，如果不加，产生的文件名字前会带上一个斜杠之前的内容，例如如果不加/产生文件名字会加上14-round-ND
#wdir = './freshly_trained_nets/final/'
wdir = './freshly_trained_student_nets_mistake/14-round-ND/'
diff = (0x0, 0x2)
bits = [47, 46, 45, 44, 43, 42, 41, 40, 39, 38, 37, 36, 35, 34, 33, 32, 0]
teacher = './freshly_trained_nets/key-recovery/14-round-ND/best14depth5-(0x0,2).h5'
...
def make_resnet(num_blocks=2, num_filters_1=48, num_filters_2=48, num_filters_3=48, num_outputs=1, d0=512,d1=64, d2=64, word_size=48, ks=3,depth=5, reg_param=0.0001, final_activation='sigmoid',s_groups = 1,ks_value_1=1,ks_value_2=3,ks_value_3=3,dropout_value=0.8,se_ratio=16):
...
def train_distinguisher(num_epochs, num_rounds=7, depth=1,filename="default",s_groups=1,bs=5000,lr_choice="14",d0=512,d1=64,d2=64,ks_value_1=1,ks_value_2=3,ks_value_3=3,machine_name='vgc',dropout_value=0.8,num_filters_1=48,num_filters_2=48,num_filters_3=48,reg_param=10**-4,high_lr=0.002,low_lr=0.0001,lr_epoch=10,se_ratio=16):
    ...
    X, Y = sp.make_train_data(10**6*2, num_rounds, diff=diff, s_groups=s_groups)   #减少一些数据量,由10**7*2将为10**6*2
    ...
    # generate eval data
    X_eval, Y_eval = sp.make_train_data(10**5*2, num_rounds, diff=diff, s_groups=s_groups)  #减少一些数据量,由10**6*2将为10**5*2
