...
#bs = 1000
wdir = './freshly_trained_student_nets/14-round-ND_simple/'
diff = (0x0, 0x1)
bits = [23, 22, 21, 20, 19, 18, 17, 16, 15, 14, 13, 12]
teacher = './freshly_trained_nets/14-round-ND/best14depth5(0x0,1).h5'
...
def make_resnet(num_blocks=2, num_filters_1=24, num_filters_2=24, num_filters_3=24, num_outputs=1, d0=512,d1=48, d2=48, word_size=24, ks=3, depth=5, reg_param=0.0001, final_activation='sigmoid',s_groups = 1,ks_value_1=1,ks_value_2=3,ks_value_3=3,dropout_value=0.8,se_ratio=16):
...
def train_distinguisher(num_epochs, num_rounds=7, depth=1, filename="default", s_groups=1, bs=5000, lr_choice="14", d0=512, d1=48, d2=48, ks_value_1=1, ks_value_2=3, ks_value_3=3, machine_name='vgc', dropout_value=0.8, num_filters_1=24, num_filters_2=24, num_filters_3=24, reg_param=10**-4,high_lr=0.002,low_lr=0.0001,lr_epoch=10,se_ratio=16): #23个参数
    #print(diff)
    #print(bits)
    ...
    X, Y = sp.make_train_data(10**7, num_rounds, diff=diff, s_groups=s_groups)   #减少一些数据量,由10**7*2将为10**7
    ...
    # generate eval data
    X_eval, Y_eval = sp.make_train_data(10**6, num_rounds, diff=diff, s_groups=s_groups)  #减少一些数据量,由10**6*2将为10**6
    ...
    #net.compile(optimizer='Adam', loss='binary_crossentropy', metrics=['acc', tf.keras.metrics.TruePositives(), tf.keras.metrics.TrueNegatives(), tf.keras.metrics.FalsePositives(),tf.keras.metrics.FalseNegatives()])
    ...
    #原代码中使用的损失函数为loss='mse' 均方误差，因为16轮出问题，尝试将损失函数改为binary_crossentropy
