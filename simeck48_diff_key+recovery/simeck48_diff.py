import numpy as np
from os import urandom


def WORD_SIZE():
    return(24)

def ALPHA():
    return(0)

def BETA():
    return(5)

def Gamma():
    return(1)

MASK_VAL = 2 ** WORD_SIZE() - 1
C = 2 ** WORD_SIZE() - 4

def make_zj(t):
    zs = [0 for i in range(t)]
    for i in range(0, 5):
        zs[i] = 1
    for i in range(5, 32):
        zs[i] = zs[i-3] ^ zs[i-5]
    for i in range(32, t):
        zs[i] = 1
    return zs

def rol(x,k):
    return(((x << k) & MASK_VAL) | (x >> (WORD_SIZE() - k))) 


def enc_one_round_simeck(p, k):
    c1 = p[0] 
    c0 = (rol(p[0], 5) & rol(p[0],0)) ^ rol(p[0],1) ^ p[1] ^ k
    return(c0,c1)

def dec_one_round_simeck(c,k):
    c0, c1 = c[0], c[1]
    c0 = c0 ^ k ^ rol(c1, Gamma()) ^ (rol(c1, ALPHA()) & rol(c1, BETA()))
    return(c1, c0)


def expand_key_simeck(k, ts):
    ks = [0 for i in range(ts)]
    t = [0 for i in range(ts+2)]
    ks[0] = k[len(k) - 1]
    zs=make_zj(36)
    for i in range(3):
        t[i] = k[2-i]
    for i in range(ts-1):
        ks[i+1] = t[i]
        t[i+3], ks[i+1] = enc_one_round_simeck((t[i], ks[i]), C ^ zs[i % 32])
    return (ks)


def encrypt_simeck(p, ks):
    x, y = p[0], p[1]
    for k in ks:
        x,y = enc_one_round_simeck((x, y), k)
    return(x, y)

def decrypt_simeck(c, ks):
    x, y = c[0], c[1]
    for k in reversed(ks):
        x, y = dec_one_round_simeck((x, y), k)
    return (x, y)



def convert_to_binary(arr, s_groups=1):
  X = np.zeros((8 * WORD_SIZE() * s_groups, len(arr[0])), dtype=np.uint8)
  for i in range(8 * WORD_SIZE() * s_groups):
    index = i // WORD_SIZE() 
    offset = WORD_SIZE() - (i % WORD_SIZE()) - 1
    X[i] = (arr[index] >> offset) & 1
  X = X.transpose() 
  return(X)


def make_train_data(n, nr, diff=(0x0, 0x1), s_groups=1):
    Y = np.frombuffer(urandom(n), dtype=np.uint8) & 1
    keys = np.random.randint(0, 2 ** 24, size=(4, n), dtype=np.uint32)
    num_rand_samples = np.sum(Y == 0)
    ks = expand_key_simeck(keys, nr)
    X_result = []

    for i in range(s_groups):

        plain0l = np.random.randint(0, 2 ** 24, size=n, dtype=np.uint32)
        plain0r = np.random.randint(0, 2 ** 24, size=n, dtype=np.uint32)

        plain1l = plain0l ^ diff[0]
        plain1r = plain0r ^ diff[1]

        plain1l[Y == 0] = np.random.randint(0, 2**24, size=num_rand_samples, dtype=np.uint32)
        plain1r[Y == 0] = np.random.randint(0, 2**24, size=num_rand_samples, dtype=np.uint32)

        ctdata0l, ctdata0r = encrypt_simeck((plain0l, plain0r), ks)
        ctdata1l, ctdata1r = encrypt_simeck((plain1l, plain1r), ks)
        
        delta_ctdata0l = ctdata0l ^ ctdata1l
        delta_ctdata0r = ctdata0r ^ ctdata1r
        
        delta_ctdata0rr = ctdata0l ^ ctdata1l ^ ctdata0r ^ ctdata1r
        
        delta_ctdata0 = ctdata0l ^ ctdata0r
        delta_ctdata1 = ctdata1l ^ ctdata1r

        secondLast_ctdata0r = rol(ctdata0r, 5) & rol(ctdata0r, 0) ^ rol(ctdata0r, 1) ^ ctdata0l
        secondLast_ctdata1r = rol(ctdata1r, 5) & rol(ctdata1r, 0) ^ rol(ctdata1r, 1) ^ ctdata1l
        
 
        delta_secondLast_ctdata0r =  secondLast_ctdata0r ^ secondLast_ctdata1r
        
        
        thirdLast_ctdata0r = ctdata0r ^ rol(secondLast_ctdata0r,5) & rol(secondLast_ctdata0r,0) ^ rol(secondLast_ctdata0r,1)
        thirdLast_ctdata1r = ctdata1r ^ rol(secondLast_ctdata1r,5) & rol(secondLast_ctdata1r,0) ^ rol(secondLast_ctdata1r,1)
        
        
        delta_thirdLast_ctdata0r = thirdLast_ctdata0r ^ thirdLast_ctdata1r


        X_result.append(delta_ctdata0l)
        X_result.append(delta_ctdata0r)

        X_result.append(ctdata0l)
        X_result.append(ctdata0r)
        
        X_result.append(ctdata1l)
        X_result.append(ctdata1r)

        X_result.append(delta_ctdata0rr)
        X_result.append(delta_secondLast_ctdata0r)

    
    X= convert_to_binary(X_result, s_groups=s_groups)
    return (X, Y)


def make_target_key_bit_diffusion_data(ks, id=0):
     ks = ks ^ 2**id
     return ks

def make_train_data_key(n, nr, diff=(0x0, 0x800000), s_groups=1, id=0):
    Y = np.frombuffer(urandom(n), dtype=np.uint8) & 1
    keys = np.random.randint(0, 2 ** 24, size=(4, n), dtype=np.uint32)

    num_rand_samples = np.sum(Y == 0)
    ks = expand_key_simeck(keys, nr+1)
    X_result = []

    for i in range(s_groups):

        plain0l = np.random.randint(0, 2 ** 24, size=n, dtype=np.uint32)
        plain0r = np.random.randint(0, 2 ** 24, size=n, dtype=np.uint32)

        plain1l = plain0l ^ diff[0]
        plain1r = plain0r ^ diff[1]

        plain1l[Y == 0] = np.random.randint(0, 2 ** 24, size=num_rand_samples, dtype=np.uint32)
        plain1r[Y == 0] = np.random.randint(0, 2 ** 24, size=num_rand_samples, dtype=np.uint32)

        ctdata0l0, ctdata0r0 = encrypt_simeck((plain0l, plain0r), ks)
        ctdata1l0, ctdata1r0 = encrypt_simeck((plain1l, plain1r), ks)

        ks[nr] = make_target_key_bit_diffusion_data(ks[nr], id)

        ctdata0l, ctdata0r = dec_one_round_simeck((ctdata0l0, ctdata0r0), ks[nr])
        ctdata1l, ctdata1r = dec_one_round_simeck((ctdata1l0, ctdata1r0), ks[nr])


        delta_ctdata0l = ctdata0l ^ ctdata1l
        delta_ctdata0r = ctdata0r ^ ctdata1r

        delta_ctdata0rr = ctdata0l ^ ctdata1l ^ ctdata0r ^ ctdata1r

        delta_ctdata0 = ctdata0l ^ ctdata0r
        delta_ctdata1 = ctdata1l ^ ctdata1r

        secondLast_ctdata0r = rol(ctdata0r, 5) & rol(ctdata0r, 0) ^ rol(ctdata0r, 1) ^ ctdata0l
        secondLast_ctdata1r = rol(ctdata1r, 5) & rol(ctdata1r, 0) ^ rol(ctdata1r, 1) ^ ctdata1l

        delta_secondLast_ctdata0r = secondLast_ctdata0r ^ secondLast_ctdata1r

        thirdLast_ctdata0r = ctdata0r ^ rol(secondLast_ctdata0r, 5) & rol(secondLast_ctdata0r, 0) ^ rol(
            secondLast_ctdata0r, 1)
        thirdLast_ctdata1r = ctdata1r ^ rol(secondLast_ctdata1r, 5) & rol(secondLast_ctdata1r, 0) ^ rol(
            secondLast_ctdata1r, 1)

        delta_thirdLast_ctdata0r = thirdLast_ctdata0r ^ thirdLast_ctdata1r

        X_result.append(delta_ctdata0l)
        X_result.append(delta_ctdata0r)

        X_result.append(ctdata0l)
        X_result.append(ctdata0r)

        X_result.append(ctdata1l)
        X_result.append(ctdata1r)

        X_result.append(delta_ctdata0)
        X_result.append(delta_ctdata1)

    X = convert_to_binary(X_result, s_groups=s_groups)

    return (X, Y)

