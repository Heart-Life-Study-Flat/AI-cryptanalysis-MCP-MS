...
teacher = './freshly_trained_nets/11-round-ND/best11depth5(0x0,800000).h5'
...
def extract_sensitive_bits(raw_x, bits=[20, 19, 18, 17, 16, 15, 14, 13, 12, 11, 10, 9, 8]):
    ...
    id1 = [v + sp.WORD_SIZE() * i for i in range(32) for v in id0]
    new_x = raw_x[:, id1]

    return new_x


def train_distinguisher(num_epochs, num_rounds=7, depth=1,filename="default",s_groups=1,bs=5000,lr_choice="14",d0=512,d1=48,d2=48,ks_value_1=1,ks_value_2=3,ks_value_3=3,machine_name='vgc',dropout_value=0.8,num_filters_1=24,num_filters_2=24,num_filters_3=24,reg_param=10**-4,high_lr=0.002,low_lr=0.0001,lr_epoch=10,se_ratio=16):
    teacher_net = load_model(teacher)

    X, Y = sp.make_train_data(10**7, num_rounds, diff=diff, s_groups=s_groups)
