...
def expand_key_simon(k, t):
    fs = 0b01011010000110010011111011100010101101000011001001111101110001
    ks = []
...
def make_train_data(n, nr, diff=(0x0, 0x800000), s_groups=1):
...
def make_train_data_key(n, nr, diff=(0x0, 0x800000), s_groups=1,id=0):
    ...
    for i in range(s_groups):
        ...
        ctdata0l, ctdata0r = dec_one_round_simon((ctdata0l0, ctdata0r0), ks[nr])
        ctdata1l, ctdata1r = dec_one_round_simon((ctdata1l0, ctdata1r0), ks[nr])
...
def make_train_data_block_half_key(n, nr, diff=(0x0, 0x800000), s_groups=1,id=[0]):
    Y = np.frombuffer(urandom(n), dtype=np.uint8)
    Y = Y & 1
    ...
    for i in range(s_groups):
        ...
        for j in range(len(id)):
            ks[nr] = make_target_key_bit_diffusion_data(ks[nr], id[j])
...
def make_train_data_block_all_key(n, nr, diff=(0x0, 0x800000), s_groups=1,id=[0]):
    Y = np.frombuffer(urandom(n), dtype=np.uint8)
    Y = Y & 1
    keys = np.random.randint(0, 2 ** 24, size=(4, n), dtype=np.uint32)
    num_rand_samples = np.sum(Y == 0)
    ...
    X_result = []
    for j in range(len(id)):
        ksnr = make_target_key_bit_diffusion_data(ks[nr], id[j])
    ...
    for i in range(s_groups):
        ...
        ctdata0l, ctdata0r = dec_one_round_simon((ctdata0l0, ctdata0r0), ksnr)
        ctdata1l, ctdata1r = dec_one_round_simon((ctdata1l0, ctdata1r0), ksnr)
